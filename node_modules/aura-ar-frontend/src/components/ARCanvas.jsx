...
function interpolateLandmark(a, b, amount) {
  return {
...
function offsetLandmark(point, xPixels, yPixels, width, height) {
  return {
    x: point.x + xPixels / Math.max(width, 1),
    y: point.y + yPixels / Math.max(height, 1),
    z: point.z ?? 0
  };
}
...
function normalizeDetectorKeypoint(point, width, height) {
  return {
    x: point.x > 1 ? point.x / Math.max(width, 1) : point.x,
    y: point.y > 1 ? point.y / Math.max(height, 1) : point.y,
    z: point.z ?? 0
  };
}

function getKeypointByLabel(keypoints, label) {
  const normalizedLabel = label.toLowerCase();

  return keypoints.find((point) =>
    String(point.label ?? point.categoryName ?? point.name ?? "")
      .toLowerCase()
      .includes(normalizedLabel)
  );
}

function getDetectorTragions(faceDetection, width, height) {
  const keypoints = faceDetection?.keypoints;

  if (!Array.isArray(keypoints) || keypoints.length < 6) {
    return null;
  }

  const rightTragion =
    getKeypointByLabel(keypoints, "rightEarTragion") ??
    getKeypointByLabel(keypoints, "right ear tragion") ??
    keypoints[4];

  const leftTragion =
    getKeypointByLabel(keypoints, "leftEarTragion") ??
    getKeypointByLabel(keypoints, "left ear tragion") ??
    keypoints[5];

  if (!leftTragion || !rightTragion) {
    return null;
  }

  const tragions = [
    normalizeDetectorKeypoint(leftTragion, width, height),
    normalizeDetectorKeypoint(rightTragion, width, height)
  ].sort((a, b) => a.x - b.x);
...
function isEarVisible(earlobe, faceDetection, side) {
  if (!earlobe) {
    return false;
  }

  if (faceDetection?.keypoints?.length >= 6) {
    const tragionIndex = side === "left" ? 5 : 4;
    const tragion = faceDetection.keypoints[tragionIndex];

    if (tragion && "score" in tragion) {
      return tragion.score >= EAR_VISIBILITY_THRESHOLD;
    }
  }
...
function calculateFullHeadPose(face, width, height, faceDetection) {
  const metrics = calculateFaceMetrics(face, width, height, faceDetection);
  ...
  const yaw = (nose.x - faceCenterX) / earSpan;
  const pitch = (nose.y - eyeCenterY) / earSpan - 0.24;
...
function calculateEarlobeMetrics(face, width, height, metrics, faceDetection) {
  const detectorTragions = getDetectorTragions(faceDetection, width, height);

  if (detectorTragions) {
    const earOutwardOffset = metrics.faceWidth * EAR_OUTWARD_OFFSET;
    const earDownwardOffset = metrics.faceHeight * EAR_DOWNWARD_OFFSET;

    const leftLobe = offsetLandmark(
      detectorTragions.left,
      -earOutwardOffset,
      earDownwardOffset,
      width,
      height
    ...
    const rightLobe = offsetLandmark(
      detectorTragions.right,
      earOutwardOffset,
      earDownwardOffset,
      width,
      height
    ...
    const leftLength = distanceBetween(metrics.leftEar, leftLobe, width, height);
    const rightLength = distanceBetween(metrics.rightEar, rightLobe, width, height);
    const averageLength = (leftLength + rightLength) / 2;
    const lobeDistance = distanceBetween(leftLobe, rightLobe, width, height);
    ...
    return {
      ...
      rightLength,
      averageLength,
      distance: lobeDistance,
      distanceToFaceWidth: lobeDistance / Math.max(metrics.faceWidth, 1),
      lengthToFaceHeight: averageLength / Math.max(metrics.faceHeight, 1),
      source: "face-detector-tragion",
      leftEarVisible: isEarVisible(leftLobe, faceDetection, "left"),
      rightEarVisible: isEarVisible(rightLobe, faceDetection, "right")
    };
  }

  const leftEar = metrics.leftEar;
  const rightEar = metrics.rightEar;

  const leftLowerJaw = landmarkPoint(face, 172, landmarkPoint(face, 136, leftEar));
  const rightLowerJaw = landmarkPoint(face, 397, landmarkPoint(face, 365, rightEar));

  const leftLobeInterp = interpolateLandmark(leftEar, leftLowerJaw, 0.70);
  const rightLobeInterp = interpolateLandmark(rightEar, rightLowerJaw, 0.70);

  const earOutwardOffset = metrics.faceWidth * EAR_OUTWARD_OFFSET;
  const earDownwardOffset = metrics.faceHeight * EAR_DOWNWARD_OFFSET;

  const leftLobe = offsetLandmark(leftLobeInterp, -earOutwardOffset, earDownwardOffset, width, height);
  const rightLobe = offsetLandmark(rightLobeInterp, earOutwardOffset, earDownwardOffset, width, height);

  const leftLength = distanceBetween(leftEar, leftLobe, width, height);
  const rightLength = distanceBetween(rightEar, rightLobe, width, height);
  const averageLength = (leftLength + rightLength) / 2;
  const lobeDistance = distanceBetween(leftLobe, rightLobe, width, height);
...
/* =========================================================
   EAR ANCHOR SMOOTHING
   ========================================================= */

function smoothEarlobeMetrics(previous, next) {
  if (!previous || previous.source !== next.source) {
    return next;
  }

  const smoothPoint = (from, to) => interpolateLandmark(from, to, EAR_ANCHOR_SMOOTHING);

  return {
    ...next,
    leftLobe: smoothPoint(previous.leftLobe, next.leftLobe),
    rightLobe: smoothPoint(previous.rightLobe, next.rightLobe),
    leftLength: previous.leftLength + (next.leftLength - previous.leftLength) * EAR_ANCHOR_SMOOTHING,
    rightLength: previous.rightLength + (next.rightLength - previous.rightLength) * EAR_ANCHOR_SMOOTHING,
    averageLength: previous.averageLength + (next.averageLength - previous.averageLength) * EAR_ANCHOR_SMOOTHING,
    distance: previous.distance + (next.distance - previous.distance) * EAR_ANCHOR_SMOOTHING,
    distanceToFaceWidth: previous.distanceToFaceWidth + (next.distanceToFaceWidth - previous.distanceToFaceWidth) * EAR_ANCHOR_SMOOTHING,
    lengthToFaceHeight: previous.lengthToFaceHeight + (next.lengthToFaceHeight - previous.lengthToFaceHeight) * EAR_ANCHOR_SMOOTHING
  };
}
...
function calculateFaceMetrics(face, width, height, faceDetection) {
  ...
  const metrics = {
    leftEar, rightEar, leftCheek, rightCheek, leftEye, rightEye, nose, chin,
    faceWidth, faceHeight, interpupillaryDistance, facialIndex, ratioScale, widthScale, roll,
    center: midpoint(leftCheek, rightCheek, width, height)
  };

  return {
    ...metrics,
    earlobes: calculateEarlobeMetrics(face, width, height, metrics, faceDetection)
  };
}
...
function createCalibrationSample(metrics) {
  return {
    ...
    earlobeDistanceToFaceWidth: metrics.earlobes.distanceToFaceWidth,
    earlobeLengthToFaceHeight: metrics.earlobes.lengthToFaceHeight
  };
}
...
function averageCalibrationSamples(samples) {
  const total = samples.reduce(
    (sum, sample) => ({
      ...
      interpupillaryToFaceWidth: sum.interpupillaryToFaceWidth + sample.interpupillaryToFaceWidth,
      earlobeDistanceToFaceWidth: sum.earlobeDistanceToFaceWidth + sample.earlobeDistanceToFaceWidth,
      earlobeLengthToFaceHeight: sum.earlobeLengthToFaceHeight + sample.earlobeLengthToFaceHeight
    }),
    { faceWidth: 0, faceHeight: 0, facialIndex: 0, interpupillaryToFaceWidth: 0, earlobeDistanceToFaceWidth: 0, earlobeLengthToFaceHeight: 0 }
  );
  ...
  return {
    ...
    earlobeDistanceToFaceWidth: total.earlobeDistanceToFaceWidth / samples.length,
    earlobeLengthToFaceHeight: total.earlobeLengthToFaceHeight / samples.length
  };
}
...
/* =========================================================
   ACCESSORY TRANSFORMS
   ========================================================= */

function getAccessoryTransforms(face, width, height, accessory, calibration, faceDetection) {
  const metrics = calculateFaceMetrics(face, width, height, faceDetection);
  const { leftEye, rightEye, nose, chin, faceWidth, faceHeight, ratioScale, earlobes } = metrics;

  const calibratedRatioScale = calibration
    ? clamp(calibration.facialIndex / FACE_REFERENCE.facialIndex, 0.86, 1.16)
    : ratioScale;

  const leftX = earlobes.leftLobe.x * width;
  const leftY = earlobes.leftLobe.y * height;
  const rightX = earlobes.rightLobe.x * width;
  const rightY = earlobes.rightLobe.y * height;
  ...
  const calibratedLobeLength = calibration?.earlobeLengthToFaceHeight
    ? calibration.earlobeLengthToFaceHeight * faceHeight
  ...
  return [
    { x: leftX - width / 2, y: height / 2 - leftY, drawHeight: earringHeight, rotation: -angle, mirror: -1, holder: "left", crop: "left" },
    { x: rightX - width / 2, y: height / 2 - rightY, drawHeight: earringHeight, rotation: -angle, mirror: 1, holder: "right", crop: "right" }
  ];
}
...
function getCropRect(image, crop) {
  if (crop === "left") return { sx: 0, sy: 0, sw: image.naturalWidth / 2, sh: image.naturalHeight };
  if (crop === "right") return { sx: image.naturalWidth / 2, sy: 0, sw: image.naturalWidth / 2, sh: image.naturalHeight };
...
function drawProductImage(ctx, image, transform) {
  ...
  ctx.drawImage(image, crop.sx, crop.sy, crop.sw, crop.sh, -drawWidth / 2, -drawHeight / 2, drawWidth, drawHeight);
  ctx.restore();
}
...
function normalizeEarringModel(model, targetHeight = 100) {
  ...
  model.position.y -= finalBox.max.y;
}
...
function createThreeTryOnScene(canvas) {
  ...
  return {
    renderer, scene, camera, centerAccessory,
    earrings: { left: leftEarring, right: rightEarring }
  };
}
...
function updateThreeEarrings(threeScene, face, width, height, selectedAccessory, calibration, faceDetection, pose) {
  ...
  for (const side of ["left", "right"]) {
    ...
    model.visible = true;
    model.position.set(transform.x, transform.y, -20 * Math.max(0.5, scale));
    model.scale.set(scale * transform.mirror, scale, scale);
    model.rotation.set(pose.pitch * 0.5, pose.yaw * transform.mirror * 0.8, transform.rotation);
  }
...
function updateThreeAccessory(threeScene, face, width, height, selectedAccessory, calibration, faceDetection, pose) {
  ...
  const scale = (transform.drawWidth ?? transform.drawHeight ?? 150) / 100;
  ...
  threeScene.centerAccessory.rotation.set(pose.pitch * 0.35, pose.yaw * 0.45, transform.rotation);
...
function renderProductImagePreview(ctx, image, width, height, selectedAccessory) {
  ...
  const transform = selectedAccessory.placement === "ears" ? null : {
    x: 0,
  ...
  if (transform) {
    ...
    drawProductImage(ctx, image, { x: -xOffset, y: 0, drawHeight, rotation: 0, mirror: -1, holder: "left", crop: "left" });
    drawProductImage(ctx, image, { x: xOffset, y: 0, drawHeight, rotation: 0, mirror: 1, holder: "right", crop: "right" });
  }
...
export default function ARCanvas({ selectedAccessoryId, onStatsChange }) {
  ...
  const [calibrationStatus, setCalibrationStatus] = useState({ state: "waiting", progress: 0 });

  const fpsRef = useRef({ lastFrameTime: performance.now(), frameCount: 0, fps: 0 });

  const selectedAccessory = useMemo(() => getAccessoryById(selectedAccessoryId), [selectedAccessoryId]);

  const {
    cameraError, isCameraReady, isStartingCamera, startCamera, stopCamera, switchFacingMode, facingMode
  } = useCamera(videoRef);

  useEffect(() => {
    if (!threeCanvasRef.current) return undefined;
  ...
  useEffect(() => {
      ...
      if (!selectedAccessory.modelUrl || !threeScene) {
        ...
        loader.load(
          ...
          (gltf) => {
            ...
            if (selectedAccessory.placement === "eyes") {
              const box = new THREE.Box3().setFromObject(model);
              const size = box.getSize(new THREE.Vector3());
              const center = box.getCenter(new THREE.Vector3());

              model.position.sub(center);
              model.scale.multiplyScalar(100 / (size.x || 1));
            } else {
              normalizeEarringModel(model, 100);
            }
    ...
    image.onerror = () => {
      if (!cancelled) setProductStatus("Product photo could not load");
    };
  ...
  const renderProductImage = useCallback((ctx, face, width, height, calibration, faceDetection) => {
    ...
    getAccessoryTransforms(face, width, height, selectedAccessory, calibration, faceDetection).forEach((transform) => {
      drawProductImage(ctx, image, transform);
    });
  ...
  const renderCameraPreview = useCallback(() => {
    ...
    if (threeModelReadyRef.current && threeSceneRef.current) {
      ...
      renderProductImagePreview(ctx, productImageRef.current, width, height, selectedAccessory);
    }
  }, [selectedAccessory]);
  ...
  const handleResults = useCallback((results) => {
    ...
    if (!face) {
      ...
      if (isCameraReady) renderCameraPreview();
      return;
    }
    ...
    setEarAnchorStatus(
      faceMetrics.earlobes.source === "face-detector-tragion"
        ? "Ear anchor: FaceDetector tragion"
        : "Ear anchor: FaceMesh estimate"
    );

    faceRatioRef.current = Number(faceMetrics.facialIndex.toFixed(1));

    earlobeMetricsRef.current = {
      distance: Math.round(faceMetrics.earlobes.distance),
      length: Math.round(faceMetrics.earlobes.averageLength)
    };
    ...
    if (now - fpsRef.current.lastFrameTime >= 1000) {
      ...
      onStatsChange?.({
        ...
        earlobeDistance: earlobeMetricsRef.current.distance,
        earlobeLength: earlobeMetricsRef.current.length,
  ...
  const { landmarkCount, trackingStatus } = useFaceMesh({
    videoRef, enabled: isCameraReady, onResults: handleResults
  });
  ...
  const calibrationLabel =
    calibrationStatus.state === "ready" ? "Face calibrated"
      : calibrationStatus.state === "scanning" ? `Scanning face ${calibrationPercent}%`
    ...
    <section className="overflow-hidden rounded-[2rem] border border-rose-100 bg-[#1C1117] p-4 shadow-aura">
      <div className="mb-4 flex items-center justify-between gap-3 text-white">
        <div>
          ...
          <p className="text-sm text-rose-100/80">Facial data is processed only on this device.</p>
        </div>
      ...
      <div className="relative aspect-[4/5] overflow-hidden rounded-[1.5rem] bg-black sm:aspect-video">
        ...
        <canvas ref={threeCanvasRef} className="pointer-events-none absolute inset-0 h-full w-full scale-x-[-1]" />

        {isCameraReady ? (
          <div className="absolute left-4 top-4 space-y-2">
            <div className="w-fit rounded-full bg-black/55 px-3 py-1 text-xs font-semibold text-white">{productStatus}</div>
            <div className="w-fit rounded-full bg-black/55 px-3 py-1 text-xs font-semibold text-white">{earAnchorStatus}</div>
          </div>
        ) : null}
        ...
        {isCameraReady ? (
          <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-black/60 p-3 text-white backdrop-blur">
            ...
            <div className="h-1.5 overflow-hidden rounded-full bg-white/20">
              <div className="h-full rounded-full bg-white transition-all duration-200" style={{ width: `${calibrationPercent}%` }} />
            </div>
          </div>
        ) : null}
